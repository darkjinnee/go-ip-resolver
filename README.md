...
### Получить плоский список IP адресов (text/plain)

```
GET /resolve-flat?group=<group_name>&type=<ip_type>
```
...
**Примеры:**
```bash
# Все IP адреса группы google
curl "http://localhost:8080/resolve-flat?group=google"

# Только IPv4 адреса группы youtube
curl "http://localhost:8080/resolve-flat?group=youtube&type=ipv4"

# Только IPv6 адреса группы github
curl "http://localhost:8080/resolve-flat?group=github&type=ipv6"
```

**Ответ:**
```
142.251.36.174
2a00:1450:4016:808::200e
142.251.36.164
2a00:1450:4016:808::2004
142.251.36.196
2a00:1450:4016:809::2004
142.251.36.227
2a00:1450:4016:80a::2003
```

**Особенности:**
- Возвращает `Content-Type: text/plain; charset=utf-8`
- Каждый IP адрес на отдельной строке
- Удобно для использования в скриптах и автоматизации
- Поддерживает фильтрацию по типу IP (IPv4/IPv6)

### Получить плоский список IP адресов всех групп (text/plain)

```
GET /resolve-flat-all?type=<ip_type>
```
...
**Примеры:**
```bash
# Все IP адреса всех групп
curl "http://localhost:8080/resolve-flat-all"

# Только IPv4 адреса всех групп
curl "http://localhost:8080/resolve-flat-all?type=ipv4"

# Только IPv6 адреса всех групп
curl "http://localhost:8080/resolve-flat-all?type=ipv6"
```
...
- Возвращает `Content-Type: text/plain; charset=utf-8`
- Каждый IP адрес на отдельной строке
- Объединяет IP адреса из всех групп доменов
- Удобно для получения полного списка IP всех сервисов
- Поддерживает фильтрацию по типу IP (IPv4/IPv6)

### Короткие эндпоинты для плоского списка IP

Для удобства использования доступны короткие эндпоинты без параметров:

#### Получить только IPv4 адреса всех групп

```
GET /resolve-flat-all-ipv4
```

**Пример:**
```bash
curl "http://localhost:8080/resolve-flat-all-ipv4"
```
...
#### Получить только IPv6 адреса всех групп

```
GET /resolve-flat-all-ipv6
```

**Пример:**
```bash
curl "http://localhost:8080/resolve-flat-all-ipv6"
```
...
**Особенности коротких эндпоинтов:**
- Не требуют параметров
- Эквивалентны `/resolve-flat-all?type=ipv4` и `/resolve-flat-all?type=ipv6`
- Удобны для использования в скриптах и автоматизации
- Возвращают `Content-Type: text/plain; charset=utf-8`
